...
export default function Navbar() {
  ...
  const navList = (
    <ul className="mb-4 mt-2 flex flex-col gap-2 lg:mb-0 lg:mt-0 lg:flex-row lg:items-center lg:gap-8">
      {["About", "Experience", "Projects", "LeetCode", "Contact"].map((item) => (
        <Typography
          key={item}
          as="li"
          variant="small"
  ...
  return (
    <>
      {/* Top Left Logo - Outside of Navbar */}
      <div className="fixed top-[-53px] left-2 z-[130] p-4 md:p-8 pointer-events-auto object-contain object-left invert dark:invert-0 transition-[filter] duration-300">
        <Logo />
      </div>
      ...
      <div className="fixed top-0 left-0 right-0 z-[120] px-4 py-4 md:px-8 pointer-events-none">
        <NavbarMT 
          ...
          shadow={false}
          color="transparent"
          className="mx-auto max-w-8xl px-4 md:px-6 py-2 glass rounded-3xl border-white/20 dark:border-white/5 shadow-2xl transition-all duration-300 pointer-events-auto !z-[120]"
        >
          <div className="flex items-center justify-between pointer-events-auto">
            {/* Left: Spacer (since logo is moved outside) */}
